...
function deleteLaunchOrderTemplates(userId, symbol, transaction){
    return orderTemplateModel.destroy({
        where: { userId, name: [`Launch BUY ${symbol}`, `Launch SELL ${symbol}`]},
        transaction
    })
}
...
async function getOrderTemplatesByAutomationId(userId, automationId) {
    ...
    return automation.actions.map(a => a.orderTemplate).filter(ot => ot);
}
...
module.exports = {
    ...
    getOrderTemplatesByAutomationId,
    deleteLaunchOrderTemplates
}