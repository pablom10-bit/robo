...
function insideCandle(ohlc, bars = 1) {
    if (ohlc.high.length <= bars) return { current: false, previous: false };
...
function AO(ohlc, fastPeriod = 5, slowPeriod = 34) {
    ...
    if (ohlc.high.length <= fastPeriod || ohlc.high.length <= slowPeriod) return { current: false, previous: false };
...
function KST(closes, ROCPer1 = 10, ROCPer2 = 15, ROCPer3 = 20, ROCPer4 = 30, SMAROCPer1 = 10, SMAROCPer2 = 10, SMAROCPer3 = 10, SMAROCPer4 = 15, signalPeriod = 3) {
    ...
    if ([ROCPer1, ROCPer2, ROCPer3, ROCPer4, SMAROCPer1, SMAROCPer2, SMAROCPer3, SMAROCPer4, signalPeriod].some(p => p >= closes.length))
...
function StochRSI(closes, dPeriod = 3, kPeriod = 3, rsiPeriod = 14, stochasticPeriod = 14) {
    ...
    if ([dPeriod, kPeriod, rsiPeriod, stochasticPeriod].some(p => p >= closes.length))
        return { current: false, previous: false };
