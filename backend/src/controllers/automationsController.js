...
async function scheduleAutomations(automations){
    try {
        automations.map(auto => {
            if (auto.isActive && auto.schedule)
                startAutomationExecution(auto, false);
        });
        logger('system', 'Hydra Agenda has started!');
    } catch (err) {
        throw new Error(`Can't start agenda! Err: ${err.message}`);
    }
}

async function startAutomationExecution(automation, mustSave = true) {
    automation.isActive = true;
    const plainAutomation = automation.get ? automation.get({ plain: true }) : automation;

    if (plainAutomation.schedule) {
        const scheduleTimestamp = Date.parse(plainAutomation.schedule);
        if (scheduleTimestamp > Date.now()) {

            const pingActionIndex = plainAutomation.actions.findIndex(action => action.type === actionsRepository.actionTypes.PING);
            if (pingActionIndex !== -1) {
                const pingAutomation = { ...plainAutomation };
                pingAutomation.id = plainAutomation.id + "P";
                pingAutomation.name = "PING";
                pingAutomation.actions = [plainAutomation.actions[pingActionIndex]];
                pingAutomation.schedule = new Date(scheduleTimestamp - (60 * 1000)).toISOString();
                agenda.addSchedule(pingAutomation);
                if (pingAutomation.logs) logger("A:" + pingAutomation.id, `Ping automation ${pingAutomation.id} has been scheduled!`);
            }

            const buyAutomation = { ...plainAutomation };
            buyAutomation.actions = [plainAutomation.actions[0]];
            agenda.addSchedule(buyAutomation);
            if (buyAutomation.logs) logger("A:" + buyAutomation.id, `Launch automation ${buyAutomation.name} has been scheduled!`);

            const sellAutomation = { ...plainAutomation };
            sellAutomation.schedule = null;
            sellAutomation.actions = [plainAutomation.actions[1]];
            hydra.updateBrain(sellAutomation);
        }
    }
    else
        hydra.updateBrain(plainAutomation);

    if (plainAutomation.logs) logger('A:' + plainAutomation.id, `Automation ${plainAutomation.name} has started!`);

    let savedAutomation = automation;
    if (mustSave)
        savedAutomation = await automation.save();
...
async function stopAutomationExecution(automation) {
    if (automation.schedule) {
        const hasPingAction = automation.actions.findIndex(action => action.type === actionsRepository.actionTypes.PING) !== -1;
        if (hasPingAction) {
            agenda.cancelSchedule(automation.id + "P");
            if (automation.logs) logger("A:" + automation.id + "P", `Ping automation ${automation.id}P has been unscheduled!`);
        }

        agenda.cancelSchedule(automation.id);
        if (automation.logs) logger("A:" + automation.id, `Launch automation ${automation.name} has been unscheduled!`);
    }
...
async function insertLaunchOrderTemplates(automation, transaction) {
    const otb = await orderTemplatesRepository.insertOrderTemplate({
        name: `Launch BUY ${automation.symbol}`,
        ...
        quantity: "QUOTE_QTY",
        quantityMultiplier: Number(automation.quantity),
        userId: automation.userId
    }, transaction);

    const ots = await orderTemplatesRepository.insertOrderTemplate({
        name: `Launch SELL ${automation.symbol}`,
        symbol: automation.symbol,
        type: "MARKET",
        side: "SELL",
        limitPrice: "",
...
async function insertLaunchActions(automationId, buyOrderTemplateId, sellOrderTemplateId, pingTuning, transaction) {
    const actions = [];
    actions.push({
        type: actionsRepository.actionTypes.ORDER,
        automationId,
        orderTemplateId: buyOrderTemplateId
    })

    actions.push({
        type: actionsRepository.actionTypes.ORDER,
        automationId,
        orderTemplateId: sellOrderTemplateId
    })

    if (pingTuning)
        actions.push({
            type: actionsRepository.actionTypes.PING,
            automationId,
            orderTemplateId: null
        })

    return actionsRepository.insertActions(actions, transaction);
}

async function insertLaunchAutomation(req, res, next) {
    const userId = res.locals.token.id;
    const newAutomation = req.body;
    newAutomation.name = "Launch " + newAutomation.symbol;
    newAutomation.userId = userId;

    if (!validateConditions(newAutomation.conditions) && !newAutomation.schedule)
        return res.status(422).json('You need to have at least one exit condition per launch automation and a schedule!');
    ...
    if (user.automations.length >= user.limit.maxAutomations || !user.limit.hasLaunch)
        return res.status(409).send(`You have reached the max automations in your plan or you don't have Launch Automations enabled.`);

    const transaction = await db.transaction();
    let savedAutomation;

    const pingTuning = newAutomation.pingTuning;
    delete newAutomation.pingTuning;

    try {
        const { otb, ots } = await insertLaunchOrderTemplates(newAutomation, transaction);
        delete newAutomation.quantity;

        savedAutomation = await automationsRepository.insertAutomation(newAutomation, transaction);

        await insertLaunchActions(savedAutomation.id, otb.id, ots.id, pingTuning, transaction);
    ...
    if (savedAutomation.isActive)
        await startAutomationExecution(savedAutomation);
...
async function insertAutomation(req, res, next) {
    ...
    if (!validateConditions(newAutomation.conditions) && !newAutomation.schedule)
        return res.status(422).json('You need to have at least one condition per automation!');
...
async function updateLaunchAutomation(req, res, next) {
    const userId = res.locals.token.id;
    const id = req.params.id;
    ...
    if (!validateConditions(newAutomation.conditions) && !newAutomation.schedule)
        return res.status(422).json('You need to have at least one exit condition per automation and a schedule!');
    ...
    try {
        ...
        const currentOts = await orderTemplatesRepository.getOrderTemplatesByAutomationId(userId, updatedAutomation.id);
        const orderTemplatesIds = currentOts.map(ot => ot.id);

        await actionsRepository.deleteActions(id, transaction);
        await orderTemplatesRepository.deleteOrderTemplatesByIds(userId, orderTemplatesIds, transaction);

        const { otb, ots } = await insertLaunchOrderTemplates(newAutomation, transaction);
        await insertLaunchActions(updatedAutomation.id, otb.id, ots.id, pingTuning, transaction);
    ...
    if (updatedAutomation.isActive)
        await startAutomationExecution(updatedAutomation);
    else
        await stopAutomationExecution(updatedAutomation);
...
async function updateAutomation(req, res, next) {
    ...
    if (!newAutomation.actions || !newAutomation.actions.length)
        return res.status(422).json('You need to have at least one action per automation!');
...
async function deleteAutomation(req, res, next) {
    ...
    if (currentAutomation.isActive)
        await stopAutomationExecution(currentAutomation);
    ...
    try {
        ...
        const isLaunchAutomation = currentAutomation.name === `Launch ${currentAutomation.symbol}`;
        if (isLaunchAutomation)
            await orderTemplatesRepository.deleteLaunchOrderTemplates(userId, currentAutomation.symbol, transaction);
